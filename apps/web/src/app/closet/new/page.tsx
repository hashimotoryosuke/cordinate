...
export default function NewClosetPage(): React.JSX.Element {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      // 1. Get presigned URL — include contentLength as required by server schema
      const presigned = await apiRequest<PresignedUrlResponse>('/upload/presigned-url', {
        method: 'POST',
        token: accessToken,
        body: JSON.stringify({
      ...
      await apiRequest<{ data: ClothingItem }>('/closet', {
        method: 'POST',
        token: accessToken,
        body: JSON.stringify({ imageUrl: presigned.data.imageUrl }),
      })
  ...
  return (
    <div className="flex min-h-screen flex-col" style={{ backgroundColor: 'var(--color-background)' }}>
      ...
      >
        <Link href="/closet" className="flex items-center gap-1 text-sm" style={{ color: 'var(--color-muted-foreground)' }}>
      ...
      <main className="flex-1 px-4 py-6">
        <div className="mx-auto max-w-sm">
          <div className="mb-6">
            ...
            <p className="mt-1 text-sm" style={{ color: 'var(--color-muted-foreground)' }}>AIが自動でタグ付けします</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-6">
            {!previewUrl ? (
              ...
              >
                ...
                <span className="text-xs" style={{ color: 'var(--color-muted-foreground)' }}>JPEG / PNG / WEBP</span>
              </button>
            ) : (
            ...
            <button
              type="submit"
              disabled={!file || isUploading || !accessToken}
